...
export default function Contribute() {
  ...
  const [{topic,subTopic},setData]= useState({topic:"Operating System",subTopic:"Threads"});
  const [{qtopic,level},setData1]= useState({qtopic:"Data Structures",level:"Easy"});
  ...
  const pTOptions = [
    { name: "Operating System", value: "6oDOgLyviDBtuKKgsYKw",subTopics:[
    ...
    { name: "Data Structure And Algorithms", value: "cY1T9XTiPw5ZeQSajOym",subTopics:[
    ...
    { name: "Networking", value: "spNoWcCvYPVbVJYz677o",subTopics:[
    ...
    { name: "DBMS", value: "xhEdXgM9qAYBGA5OBXCU",subTopics:[
  ...
  const qTOptions = [
    ...
    },
    {
      ...
      levels:[
        { name: "Easy", value: "0QT8D7xT5armmBvh319g" },
      ]
    },
  ];
  ...
  const subTopics = pTOptions.find(item=>item.name===topic)?.subTopics.map(option=>(
    <option key={option.name} value={option.name}>{option.name}</option>
  ))
  ...
  const levels = qTOptions.find(item=>item.name===qtopic)?.levels.map(option=>(
    <option key={option.name} value={option.name}>{option.name}</option>
  ))
  ...
  async function addPracticeItem(e) {
    e.preventDefault();
    if(pTOptions.find(item=>item.name===topic).subTopics.find(st=>st.name===subTopic)!==undefined){
      setShow1(false)
    ...
    const payload = {
      ...
      sub_topic_name: pTOptions.find(item=>item.name===topic).subTopics.find(st=>st.name===subTopic).name,
      sub_topic_id: pTOptions.find(item=>item.name===topic).subTopics.find(st=>st.name===subTopic).value,
      title: title,
      link: link,
  ...
  async function addTestItem(e) {
    e.preventDefault();
    const { formTestQuestion,formTestOption1,formTestOption2,formTestOption3,formTestOption4 } = e.target.elements;
    ...
    const payload = {
      ...
      level_id: qTOptions.find(item=>item.name===qtopic).levels.find(l=>l.name===level)?.value,
      level_name:qTOptions.find(item=>item.name===qtopic).levels.find(l=>l.name===level)?.name,
          ...
          >
              ...
              <Form onSubmit={addPracticeItem} id="PracticeForm">
                <div className="mx-auto">
                  ...
                  >
                    ...
                    <Form.Select value={topic} aria-label="Default select example" onChange={handleTopicChange}>
                      {topics}
</Form.Select>
                  </Form.Group>
                  <Form.Group
                  ...
                  >
                    ...
                    <Form.Select value={subTopic} aria-label="Default select example" onChange={handleSubTopicChange}>
                      {subTopics}
</Form.Select>
                  </Form.Group>
                </div>
                <Form.Group
                  ...
                  >
                    ...
                    <Form.Control type="text" onChange={(q)=>setTitle(q.target.value)} placeholder="Enter Title " required />
                  </Form.Group>
                <Form.Group
                  className=" mb-3  ms-5 me-5"
                ...
                <Alert className="w-65 mx-auto" variant="danger" show={show1} onClose={()=>{setShow1(false)}} dismissible>Please select valid subtopic!!</Alert>
              </Form>
            </Tab>
            ...
            <Form onSubmit={addTestItem} id="TestForm">
                <div className="mx-auto">
                  ...
                  >
                    <Form.Label>Topic</Form.Label>
                    <Form.Select aria-label="Default select example" onChange={handleQTopicChange}>
                      {qtopics}
</Form.Select>
                  </Form.Group>
                  <Form.Group
